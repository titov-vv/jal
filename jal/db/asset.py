...
class JalAsset(JalDB):
    ...
    def symbol(self, currency: int = None) -> str:
        if currency is None:
            ...
            symbols = []
            while query.next():
                symbols.append(self._read_sql_record(query))
    ...
    def quotes(self, begin: int, end: int, currency_id: int) -> list:
        ...
        while query.next():
            timestamp, quote = self._read_sql_record(query)
    ...
    def get_sources_list(cls) -> dict:
        ...
        while query.next():
            source_id, name = cls._read_sql_record(query)
    ...
    def get_active_assets(cls, begin: int, end: int) -> list:
        ...
        while query.next():
            try:
                _id, asset_id, currency_id = super(JalAsset, JalAsset)._read_sql_record(query)
    ...
    def get_currencies(cls) -> list:
        ...
        while query.next():
            currencies.append(JalAsset(int(super(JalAsset, JalAsset)._read_sql_record(query))))
        return currencies
