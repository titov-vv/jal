...
CREATE TABLE asset_payments (
    ...
    amount     TEXT    NOT NULL DEFAULT ('0'),        -- Sum of money that was paid, in the account's own currency
...
CREATE TABLE ledger (
    ...
    amount_acc   TEXT,
    value_acc    TEXT,
    peer_id      INTEGER REFERENCES agents (id) ON DELETE NO ACTION ON UPDATE NO ACTION,     -- peer that is related with transaction. No FK enforcement as table is cleaned by agents_after_delete trigger
...
INSERT INTO settings(name, value) VALUES('DlgGeometry_Residence', '');
