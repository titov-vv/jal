...
from jal.widgets.helpers import (ManipulateDate, TableSelectionRestorer, set_grids_metrics,
...
class OperationsWidget(MdiWidget):
    ...
    def balances_context_menu(self, pos):
        ...
        actionGroupByTag = QAction(self.tr("Group by tag"), self)
        actionGroupByTag.setCheckable(True)
        actionGroupByTag.setChecked(JalSettings().getValue("GroupBalancesByTag", False))
        actionGroupByTag.toggled.connect(self.ui.BalancesTreeView.model().groupByTag)
        contextMenu.addAction(actionGroupByTag)
        contextMenu.addSeparator()
