...
class StatementTvoyBroker(StatementXLS):
    ...
    def load_futures_deals(self):
        ...
        while row < self._statement.shape[0]:
            ...
            symbol_id = self.symbol_id({'symbol': self._statement[headers['symbol']][row],
    ...
    def load_asset_movements(self):
        ...
        while row < self._statement.shape[0]:
            ...
            if operations[operation] is not None:
                operations[operation](timestamp, number, symbol_id, qty, description)
            cnt += 1
            row += 1
        logging.info(self.tr("Asset operations loaded: ") + f"{cnt}")

    def asset_cancellation(self, timestamp, number, symbol, qty, description):
        # Statement has negative value for cancellation - will be used to create sell trade
        record = {"timestamp": timestamp, "symbol": symbol, "number": number, "quantity": qty, "note": description}
        self.asset_withdrawal.append(record)

    def asset_transfer(self, timestamp, number, symbol, qty, description):
    ...
    def dividend(self, timestamp, number, account_id, amount, description):
        ...
        if isin_match:
            symbol_id = self.symbol_id({'isin': dividend_data['REG_NUMBER'], 'currency': currency_code, 'search_online': "MOEX"})
        else:
            symbol_id = self.symbol_id({'reg_number': dividend_data['REG_NUMBER'], 'currency': currency_code, 'search_online': "MOEX"})
    ...
    def bond_repayment(self, timestamp, _number, account_id, amount, description):
        ...
        interest_data = parts.groupdict()
        symbol_id = self.symbol_id({'symbol': interest_data['NAME'], 'should_exist': True})
        match = [x for x in self.asset_withdrawal if x['symbol'] == symbol_id and x['timestamp'] == timestamp]
        if not match:
