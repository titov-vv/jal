...
3. Для учёта сумм удержанных/выплаченных брокером (комиссий, процетов и т.п.) нужно создать в меню *Data->Peers* (в русской версии *Данные->Контрагенты*) соответствующего контрагента. Для этого достаточно нажать кнопку "+", ввести имя и подтвердить сохранение кнопкой "✔"):  
![IBRK peer](https://github.com/titov-vv/jal/blob/master/docs/ru-tax-3ndfl/img/ibkr_peer.png?raw=true)  
   
4. В *jal* все транзакции привязаны к тому или иному счету. Поэтому для успешной загрузки отчёта вам нужно заранее создать счет(а) соотвествующие вашим операциям через меню *Data->Accounts* (в русской версии *Данные->Счета*). Если у вас были только торговые операции в одной валюте, то достаточно одного счёта типа *Investment* в нужной валюте. Для учёта операций ввода/вывода средств понадобится второй счёт любого типа.  
