...
Информация об ошибках, замечания и пожелания приветствуются. Помимо Github связаться со мной и задать вопросы можно по адресу [jal@gmx.ru](mailto:jal@gmx.ru?subject=%5BJAL%5D%20Tax%20report) или в [Telegram](https://t.me/jal_support).*
...
Если счёт с таким номером и валютой уже существует, то *jal* будет использовать и его новый счёт создан не будет. Для учёта операций ввода/вывода средств необходимо добавить счёт любого типа — его можно создать, когда *jal* спросит о том откуда или куда были переведены средства.  
Я создал 2 счета для примера (как указано выше — этот шаг можно было пропустить):
