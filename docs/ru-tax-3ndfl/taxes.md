...
3. Для учёта сумм удержанных/выплаченных брокером (комиссий, процентов и т.п.) нужно создать в меню *Data->Peers* (в русской версии *Данные->Контрагенты*) соответствующего контрагента. Для этого достаточно нажать кнопку "+", ввести имя и подтвердить сохранение кнопкой "✔":  
![IBRK peer](https://github.com/titov-vv/jal/blob/master/docs/ru-tax-3ndfl/img/ibkr_peer.png?raw=true)  
   
4. В *jal* все транзакции привязаны к тому или иному счету. Поэтому для успешной загрузки отчёта вам нужно заранее создать счет(а) соответствующие вашим операциям через меню *Data->Accounts* (в русской версии *Данные->Счета*). Если у вас были только торговые операции в одной валюте, то достаточно одного счёта типа *Investment* в нужной валюте. Для учёта операций ввода/вывода средств понадобится второй счёт любого типа.  
Я создал 2 счета для примера:
    - счет типа *Investment*, у которого номер и валюта будут совпадать с номером и валютой счета Interactive Brokers. В поле Bank/Broker нужно обязательно указать наименование брокера (которое было создано в п.3) - для этого нужно выбрать это поле и либо начать набирать первые буквы, либо нажать кнопку "..." для выбора двойным кликом из списка.  
Если вы хотите сохранить данные в файл программы 'Декларация', то нужно также указать страну счёта.  
В моём примере я использую U1234567 и USD:  
...
5. После инициализации база *jal* содержит 3 валюты - RUB, USD и EUR.  
...
6. Непосредственно для загрузки отчёта вам необходимо выбрать пункт меню *Import->Statement->Interactive Brokers XML* (в русской версии *Импорт->Выписка->Interactive Brokers XML*), после чего указать XML файл, который необходимо загрузить.  
![Select statement](https://github.com/titov-vv/jal/blob/master/docs/ru-tax-3ndfl/img/menu_selection.png?raw=true)  
Если ваш отчет содержит транзакции ввода/вывода денежных средств, то вы *jal* попросит вас указать какой счет нужно использовать для списания/зачисленя этих средств, например:  
...
Возможны небольшие отклонения, у меня они составляют несколько центов. Они связаны с тем, что Interactive Brokers округляют налог в отчётах. Чтобы исправить это, вы можете проголосовать по [этой ссылке](https://interactivebrokers.com/en/general/poll/featuresPoll.php?sid=15422).
...
10. С помощью *jal* вы можете подготовить как просто расчёт в виде файла Excel, так и автоматически занести данные в файл программы *"Декларация 2020"*. 
...
11. Для выполнения расчёта вам необходимо выбрать пункт меню *Reports->Tax Report \[RU]* (в русской версии *Отчёты->Налоговый отчёт \[RU]*) и заполнить параметры в появившемся диалоговом окне:  
...
12. Если вы выбирали обновление файла декларации, то вы можете теперь открыть его с помощью программы *"Декларация 2020"* и проверить, что информация была добавлена на закладку *Доходы за пределами РФ*  
![Declaration Updated](https://github.com/titov-vv/jal/blob/master/docs/ru-tax-3ndfl/img/declaration_3.png?raw=true)
    
13. [Получившийся Excel-файл с расчётом](https://github.com/titov-vv/jal/blob/master/docs/ru-tax-3ndfl/3ndfl_tax_report.xlsx) <sup>1</sup> будет содержать 7 закладок: Дивиденды, Акции, Облигации, ПФИ, Корпоративные события, Комиссии и Проценты. 
