...
def test_ibkr_json_import(tmp_path, project_root, data_path, prepare_db_ibkr):
    ...
    assert JalDB._readSQL("SELECT COUNT(*) FROM asset_data") == len(test_asset_data)
    ...
    assert JalDB._readSQL("SELECT COUNT(*) FROM accounts") == len(test_accounts)
    for i, account in enumerate(test_accounts):
        assert JalDB._readSQL("SELECT * FROM accounts WHERE id=:id", [(":id", i+1)]) == account
    ...
    assert JalDB._readSQL("SELECT COUNT(*) FROM agents") == len(test_peers)
    for i, peer in enumerate(test_peers):
        assert JalDB._readSQL("SELECT * FROM agents WHERE id=:id", [(":id", i + 1)]) == peer
    ...
    assert JalDB._readSQL("SELECT COUNT(amount) FROM action_details") == len(test_actions)
    for i, action in enumerate(test_actions):
        assert JalDB._readSQL("SELECT * FROM action_details WHERE id=:id", [(":id", i+1)]) == action
    ...
    assert JalDB._readSQL("SELECT COUNT(*) FROM transfers") == len(test_transfers)
    for i, transfer in enumerate(test_transfers):
        assert JalDB._readSQL("SELECT * FROM transfers WHERE id=:id", [(":id", i+1)]) == transfer
    ...
    assert JalDB._readSQL("SELECT COUNT(*) FROM dividends") == len(test_payments)
    for i, payment in enumerate(test_payments):
        assert JalDB._readSQL("SELECT * FROM dividends WHERE id=:id", [(":id", i + 1)]) == payment
    ...
    assert JalDB._readSQL("SELECT COUNT(*) FROM quotes") == len(stock_quotes)
    for i, quote in enumerate(stock_quotes):
        assert JalDB._readSQL("SELECT * FROM quotes WHERE id=:id", [(":id", i + 1)]) == quote
