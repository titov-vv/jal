...
def test_spin_off(prepare_db_fifo):
    ...
    assert readSQL("SELECT * FROM ledger WHERE asset_id=4 AND timestamp<1628615520 ORDER BY id DESC LIMIT 1") == [11, 1627819200, 5, 2, 4, 4, 1, '13.0', '1452.000000000', '13.00', '1452.000000000', '', '', '']
    assert readSQL("SELECT * FROM ledger WHERE asset_id=5 AND timestamp<1628615520 ORDER BY id DESC LIMIT 1") == [7, 1622548800, 5, 1, 4, 5, 1, '5.0', '0E-9', '5.0', '0E-9', '', '', '']
    assert readSQL("SELECT * FROM ledger WHERE book_account=3 AND timestamp<1628615520 ORDER BY id DESC LIMIT 1") == [8, 1625140800, 3, 2, 3, 2, 1, '-52.000', '0.0', '8548.000', '0.0', '', '', '']
...
def test_symbol_change(prepare_db_fifo):
    ...
    assert readSQL("SELECT * FROM deals_ext WHERE asset_id=4") == [1, 'Inv. Account', 4, 'A', 1619870400, 1622548800, '10.0', '10.0', '100.00', 0.0, 0.0, 0.0, -3]
    assert readSQL("SELECT * FROM deals_ext WHERE asset_id=5") == [1, 'Inv. Account', 5, 'B', 1622548800, 1625140800, '10.00000000', '20.0', '100.00', 0.0, 1000.0, 100.0, 3]
...
def test_delisting(prepare_db_fifo):
    ...
    assert readSQL("SELECT * FROM deals_ext WHERE asset_id=4") == [1, 'Inv. Account', 4, 'A', 1619870400, 1622548800, '10.0', '10.0', '100.00', 0.0, 0.0, 0.0, -5]
    assert readSQL("SELECT * FROM ledger_totals WHERE asset_id=4 ORDER BY id DESC LIMIT 1") == [5, 5, 1, 1622548800, 4, 4, 1, '0.00', '0E-8']
    assert readSQL("SELECT * FROM ledger WHERE book_account=1") == [6, 1622548800, 5, 1, 1, 2, 1, '1000.00000000', '0.0', '1000.00000000', '0.0', 1, 9, '']
