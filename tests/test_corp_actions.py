...
def test_spin_off(prepare_db_fifo):
    ...
    assert JalDB._readSQL("SELECT * FROM ledger WHERE asset_id=4 AND timestamp<1628615520 ORDER BY id DESC LIMIT 1") == [11, 1627819200, 5, 2, 4, 4, 1, '13', '1452', '13', '1452', '', '', '']
    assert JalDB._readSQL("SELECT * FROM ledger WHERE asset_id=5 AND timestamp<1628615520 ORDER BY id DESC LIMIT 1") == [7, 1622548800, 5, 1, 4, 5, 1, '5', '0', '5', '0', '', '', '']
    assert JalDB._readSQL("SELECT * FROM ledger WHERE book_account=3 AND timestamp<1628615520 ORDER BY id DESC LIMIT 1") == [8, 1625140800, 3, 2, 3, 2, 1, '-52', '0', '8548', '0', '', '', '']
    assert JalDB._readSQL("SELECT profit FROM deals_ext WHERE close_timestamp>=1629047520") == 498.0
...
def test_symbol_change(prepare_db_fifo):
    ...
    assert JalDB._readSQL("SELECT * FROM deals_ext WHERE asset_id=4") == [1, 'Inv. Account', 4, 'A', 1619870400, 1622548800, '1E+1', '1E+1', '1E+2', 0.0, 0.0, 0.0, -3]
    assert JalDB._readSQL("SELECT * FROM deals_ext WHERE asset_id=5") == [1, 'Inv. Account', 5, 'B', 1622548800, 1625140800, '1E+1', '2E+1', '1E+2', 0.0, 1000.0, 100.0, 3]
...
def test_delisting(prepare_db_fifo):
    ...
    assert JalDB._readSQL("SELECT * FROM deals_ext WHERE asset_id=4") == [1, 'Inv. Account', 4, 'A', 1619870400, 1622548800, '1E+1', '1E+1', '1E+2', 0.0, 0.0, 0.0, -5]
    assert JalDB._readSQL("SELECT * FROM ledger_totals WHERE asset_id=4 ORDER BY id DESC LIMIT 1") == [5, 5, 1, 1622548800, 4, 4, 1, '0', '0']
    assert JalDB._readSQL("SELECT * FROM ledger WHERE book_account=1") == [6, 1622548800, 5, 1, 1, 2, 1, '1E+3', '0', '1E+3', '0', 1, 9, '']
